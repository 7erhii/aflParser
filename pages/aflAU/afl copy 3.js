...
async function fetchAFLNews() {
  ...
  const currentNews = await page.evaluate(() => {
    // Находим первый элемент с классом .media-list__list
    const firstMediaList = document.querySelectorAll('.media-list__list')[0];
    if (!firstMediaList) {
      return [];
    }
  
    // Ищем все .media-list__item только внутри первого .media-list__list
    return Array.from(firstMediaList.querySelectorAll('.media-list__item')).map(
      (item) => {
        const title =
          item.querySelector(".media-thumbnail__title")?.textContent?.trim() || "";
        const description =
          item.querySelector(".media-thumbnail__description")?.textContent?.trim() ||
          "";
        const postLink =
